...
for [{ _i = 0 }, { _i < 2 }, { _i = _i + 1 }] do 
{
	...
	{ [ _x, random [0.2, 0.75, 1] ] call trg_fnc_setSkill; } forEach (units ( veh # 2 ));
	_wp = ( _veh # 2 ) addWaypoint [ [ _trgPos # 0, _trgPos # 1, 100 ], 1000 ];
...
/*if ((playersNumber east) > 15 && (random 1) <= 0.05 ) then 
{
	...
	_veh = [_pos, random 360, "B_T_VTOL_01_armed_blue_F", blufor] call BIS_fnc_spawnVehicle;
